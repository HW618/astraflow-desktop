...
const en = {
  ...
  loginComplete: "Login complete. Redirecting to Models...",
  ...
  codeboxKill: "Kill",
  // Skills
  skillSearch: "Search plugins",
  mcpSearch: "Search MCP",
  pluginType: "Plugin type",
  pluginTypeSkills: "Plugins",
  pluginTypeMcp: "MCP",
  pluginView: "View",
  ...
  skillImportScanDescription:
    "AstraFlow scans known Codex, Claude, and Agents skill folders, then copies valid Skills into its own plugin store.",
  ...
  skillSandboxHint:
    "Enabled plugins are available to all chat sessions. The agent will call load_skill before using the full instructions and files.",
  skillNoInstalled: "No installed plugins yet.",
  skillInstalledSummary: (count: number) =>
    count === 1 ? "1 installed plugin" : `${count} installed plugins`,
  skillLatest: "Latest",
  skillNoDescription: "No plugin description is available.",
  ...
  skillsSummary: (start: number, end: number, total: number) =>
    total > 0 ? `${start}-${end} of ${total}` : "0 plugins",
  skillsPage: (page: number, totalPages: number) =>
    `Page ${page} of ${totalPages}`,
  noSkillsFound: "No plugins match the current search or filter.",
  // Studio
  ...
  studioComposerPluginsDescription:
    "Enabled plugins and MCP tools are available here.",
  ...
  studioToolListingSkills: "Listing installed plugins",
  studioToolListedSkills: "Listed installed plugins",
...
const zh: Dictionary = {
  ...
  skillImportScanDescription:
    "AstraFlow 会扫描常见 Codex、Claude 和 Agents 技能目录，并把合法 Skills 复制到自己的插件存储。",
  ...
  skillSandboxHint:
    "已启用插件会对所有聊天会话可用。智能体会先调用 load_skill，再读取完整说明和文件。",
  skillNoInstalled: "还没有安装插件。",
  skillInstalledSummary: (count: number) => `已安装 ${count} 个插件`,
  skillLatest: "最新",
  skillNoDescription: "暂无插件描述。",
  ...
  skillDownload: "下载",
  skillsSummary: (start: number, end: number, total: number) =>
    total > 0 ? `${start}-${end} / ${total}` : "0 个插件",
  skillsPage: (page: number, totalPages: number) =>
    `第 ${page} 页 / 共 ${totalPages} 页`,
  noSkillsFound: "没有匹配当前搜索或筛选条件的插件。",
  // Studio
  studioTitle: "体验",
  studioModes: "模式",
  ...
  studioToolListingSkills: "正在查看已安装插件",
  studioToolListedSkills: "已查看已安装插件",
  studioToolLoadingSkill: (input: string) =>
